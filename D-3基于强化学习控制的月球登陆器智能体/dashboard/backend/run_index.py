...
def _read_metrics(run_dir: Path, env_mode: str, exp_id: int) -> RunMetrics:
    ...
    # Backward-compat: older runs saved evaluations as sibling .npz of the run dir.
    legacy_evaluations = run_dir.parent / f"{run_dir.name}.npz"
    if not evaluations.exists() and legacy_evaluations.exists():
