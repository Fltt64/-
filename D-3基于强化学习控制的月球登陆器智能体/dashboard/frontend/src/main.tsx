...
function App() {
  ...
  const [trainMetrics, setTrainMetrics] = React.useState<RunMetrics | null>(null);
  ...
  React.useEffect(() => {
    if (!trainReferenceRun) {
      setTrainMetrics(null);
      return;
    }
    api.metrics(trainReferenceRun).then(setTrainMetrics).catch(() => setTrainMetrics(null));
  }, [trainReferenceRun]);
    ...
    <section className="page trainPage">
      ...
      <section className="monitorPanel">
        ...
        {points.length > 0 ? (
          ...
          <div className="emptyChart" style={{ height: 430 }}>暂无训练数据，请启动训练</div>
        )}
      ...
      <aside className="evalSide">
        ...
        <article className="card curveCard">
          ...
          {(metrics?.points ?? []).length > 0 ? (
            ...
            <div className="emptyChart" style={{ height: 220 }}>暂无测评数据</div>
          )}
        </article>
      </aside>
    </section>
